...
class CephFSTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data_pool_name = self.fs.get_data_pool_name() # Hacky, get the pool name for caps... (it is about to be deleted along with the file system...)
        ...
        for client_id in client_mount_ids:
            self.mds_cluster.mon_manager.raw_cluster_cmd_result(
                ...
                'mds', 'allow',
                'mon', 'allow r',
                'osd', 'allow rw pool={0}'.format(data_pool_name))
        ...
        for setting in self.LOAD_SETTINGS:
            setattr(self, setting, int(self.mds_cluster.mds_asok(
    ...
    def assert_session_count(self, expected, ls_data=None, mds_id=None):
        if ls_data is None:
            ls_data = self.mds_cluster.mds_asok(['session', 'ls'], mds_id=mds_id)
    ...
    def assert_session_state(self, client_id,  expected_state):
        self.assertEqual(
            self._session_by_id(
                self.mds_cluster.mds_asok(['session', 'ls'])).get(client_id, {'state': None})['state'],
            expected_state)
    ...
    def _session_list(self):
        ls_data = self.mds_cluster.mds_asok(['session', 'ls'])
    ...
    def get_session(self, client_id, session_ls=None):
        if session_ls is None:
            session_ls = self.mds_cluster.mds_asok(['session', 'ls'])
